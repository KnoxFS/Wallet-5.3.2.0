...
*Setup instructions for a gitian build of KFX Core using a VM or physical system.*
...
to the KFX GitHub release page.
...
- [Building KFX Core](#building-knoxfs-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/knoxfs-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/knoxfs-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/knoxfs-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/knoxfs-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/knoxfs-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/knoxfs-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/knoxfs-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@knoxfs.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/knoxfs-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/knoxfs-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/knoxfs-osx-*-build.assert
```
