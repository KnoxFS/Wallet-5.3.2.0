KFX Core version *v5.0.0* is now available from:  <https://github.com/knoxfs-project/knoxfs/releases>
...
KFX Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
For the other packages, the user must save the param files in the proper location, before being able to run KFX v5.0.0:
...
#### Overview
Users will be able to protect their financial information by sending and receiving KFXs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://knoxfs.org .
...
* Top bar showing shield KFX balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield KFX balance.
...
See the release notes from [v4.2.0](https://github.com/KFX-Project/KFX/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed KFX supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in KFX
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in KFX
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zKFXsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield KFX)
  The `moneysupply` JSON return entry is the total KFX supply (the sum of all transparent and shield KFX)
...
| `getaccountaddress` | Returned the current KFX address associated with an internal account | Account system removal |
| `setaccount` | Associate a KFX address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given KFX address | Account system removal |
| `getaddressesbyaccount` | Returned a list of KFX addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a KFX address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a KFX address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zKFX balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zKFX mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zKFX mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zKFX amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zKFX to a KFX address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zKFX mints to a KFX address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zKFXs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzpivseed` | Set the wallet's deterministic zKFX seed to a specific value | Zerocoin deprecation |
| `getzpivseed` | Return the current deterministic zKFX seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zKFX seed | Zerocoin deprecation |
| `dzpivstate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzpiv` | Do a search for deterministically generated zKFX that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_knoxfs unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake knoxfsd and unit tests: fixing missing MacOS framework. (furszy)
