UNIX BUILD NOTES
====================
Some notes on how to build KFX Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile KFX Core and the dependencies,
...
This will build knoxfs-qt as well, if the dependencies are met.
...
memory available when compiling KFX Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~knoxfs/+archive/knoxfs).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:knoxfs/knoxfs
    sudo apt-get update
...
To build KFX Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
Once these are installed, they will be found by configure and a knoxfs-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip knoxfsd" to strip the debug
...
Security
--------
To help make your KFX Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, KFX Core should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, KFX Core may be compiled in
