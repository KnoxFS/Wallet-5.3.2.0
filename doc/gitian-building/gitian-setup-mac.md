...
The build script we'll be using is contained in the KFX github repository ([contrib/gitian-build.py](https://github.com/knoxfs-project/knoxfs/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the KFX repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/KFX-Project/KFX/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, knoxfs-detached-sigs, and knoxfs GitHub repos.
