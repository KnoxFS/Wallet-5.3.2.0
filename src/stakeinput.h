...
#ifndef KFX_STAKEINPUT_H
#define KFX_STAKEINPUT_H
...
public:
    ...
    virtual bool IsZKFX() const = 0;
    virtual CDataStream GetUniqueness() const = 0;
    virtual bool ContextCheck(int nHeight, uint32_t nTime) = 0;
};


class CKnoxFSStake : public CStakeInput
{
private:
...
public:
    CKnoxFSStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CKnoxFSStake* NewKnoxFSStake(const CTxIn& txin);
    ...
    bool IsZKFX() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //KFX_STAKEINPUT_H
