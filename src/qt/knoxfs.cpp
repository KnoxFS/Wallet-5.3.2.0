...
#include "config/knoxfs-config.h"
#endif
...
#include "qt/knoxfs/splash.h"
#include "qt/knoxfs/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("knoxfs-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in knoxfs.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in knoxfs.qrc)
...
public:
    ...
    /// Get window identifier of QMainWindow (KFXGUI)
...
{
    window = new KFXGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &KFXGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &KFXGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(KFXGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(KFXGUI::DEFAULT_WALLET);
        ...
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &KFXGUI::handlePaymentRequest);
        connect(window, &KFXGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. KFX can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(knoxfs_locale);
    Q_INIT_RESOURCE(knoxfs);
    ...
    /// 6. Determine availability of data directory and parse knoxfs.conf
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("KFX Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // knoxfs: links repeatedly have their payment requests routed to this process:
