...
#include "qt/knoxfs/forms/ui_addresseswidget.h"
#include "qt/knoxfs/addresslabelrow.h"
#include "qt/knoxfs/addnewaddressdialog.h"
#include "qt/knoxfs/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(KFXGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto knoxfsAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(knoxfsAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(knoxfsAdd);
        if (walletModel->updateAddressBookLabels(knoxfsAdd, label.toUtf8().constData(),
