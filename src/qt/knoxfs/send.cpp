...
#include "qt/knoxfs/send.h"
#include "qt/knoxfs/forms/ui_send.h"
#include "qt/knoxfs/addnewcontactdialog.h"
#include "qt/knoxfs/qtutils.h"
#include "qt/knoxfs/sendchangeaddressdialog.h"
#include "qt/knoxfs/optionbutton.h"
#include "qt/knoxfs/sendconfirmdialog.h"
#include "qt/knoxfs/guitransactionsutils.h"
#include "qt/knoxfs/loadingdialog.h"
...
SendWidget::SendWidget(KFXGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-knoxfs");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onKFXSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onKFXSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent KFXs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield KFXs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent KFXs to shield."));
    }
}
...
void SendWidget::onKFXSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto knoxfsAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(knoxfsAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(knoxfsAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(knoxfsAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
