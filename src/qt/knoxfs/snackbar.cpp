...
#include "qt/knoxfs/forms/ui_snackbar.h"
#include "qt/knoxfs/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(KFXGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &KFXGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
