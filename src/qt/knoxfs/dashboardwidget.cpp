...
#include "qt/knoxfs/dashboardwidget.h"
#include "qt/knoxfs/forms/ui_dashboardwidget.h"
#include "qt/knoxfs/sendconfirmdialog.h"
#include "qt/knoxfs/txrow.h"
...
DashboardWidget::DashboardWidget(KFXGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareKnoxFS, "square-chart-knoxfs");
    //setCssProperty(ui->labelSquarezKnoxFS, "square-chart-zknoxfs");
    setCssProperty(ui->labelKnoxFS, "text-chart-knoxfs");
    //setCssProperty(ui->labelZknoxfs, "text-chart-zknoxfs");
    ...
    setCssProperty(ui->labelAmountKnoxFS, "text-stake-knoxfs-disable");
    //setCssProperty(ui->labelAmountZknoxfs, "text-stake-zknoxfs-disable");
    ui->labelAmountMNRewards->setText("0 KFX");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &KFXGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
const QMap<int, QMap<QString, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        //bool isKnoxFS = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZKFX;
        bool isKnoxFS = modelIndex.data(TransactionTableModel::TypeRole).toInt() == TransactionRecord::StakeMint;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair KFX, zKFX
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            //knoxfs = (pair.first != 0) ? pair.first / 100000000 : 0;
            //zknoxfs = (pair.second != 0) ? pair.second / 100000000 : 0;
            //chartData->totalKnoxFS += pair.first;
            //chartData->totalZknoxfs += pair.second;
            QMap<QString, qint64> pair = chartData->amountsByCache[num];
            knoxfs = (pair["knoxfs"] != 0) ? pair["knoxfs"] / 100000000 : 0;
            //zknoxfs = (pair["zknoxfs"] != 0) ? pair["zknoxfs"] / 100000000 : 0;
            mnrewards = (pair["mn"] != 0) ? pair["mn"] / 100000000 : 0;
            chartData->totalKnoxFS += pair["knoxfs"];
            //chartData->totalZknoxfs += pair["totalZknoxfs"];
            chartData->totalMNRewards += pair["mn"];
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesKnoxFS.append(knoxfs);    
        chartData->valuesMNRewards.append(mnrewards); 
        //chartData->valueszKnoxFS.append(zknoxfs);

        int max = std::max(knoxfs, mnrewards);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesKnoxFS);
    //set1->append(chartData->valueszKnoxFS);
    set1->append(chartData->valuesMNRewards);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    //if (chartData->totalKnoxFS > 0 || chartData->totalZknoxfs > 0) {
    if (chartData->totalKnoxFS > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountKnoxFS, "text-stake-knoxfs");
        //setCssProperty(ui->labelAmountZknoxfs, "text-stake-zknoxfs");
    ...
    //forceUpdateStyle({ui->labelAmountKnoxFS, ui->labelAmountZknoxfs});
    forceUpdateStyle({ui->labelAmountKnoxFS, ui->labelAmountMNRewards});
    ui->labelAmountKnoxFS->setText(GUIUtil::formatBalance(chartData->totalKnoxFS, nDisplayUnit));
    //ui->labelAmountZknoxfs->setText(GUIUtil::formatBalance(chartData->totalZknoxfs, nDisplayUnit, true));
