...
CKnoxFSStake* CKnoxFSStake::NewKnoxFSStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CKnoxFSStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new CKnoxFSStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CKnoxFSStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CKnoxFSStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CKnoxFSStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CKnoxFSStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CKnoxFSStake::GetUniqueness() const
{
    //The unique identifier for a KFX stake is the outpoint
...
const CBlockIndex* CKnoxFSStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CKnoxFSStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CKnoxFSStake::ContextCheck(int nHeight, uint32_t nTime)
{
