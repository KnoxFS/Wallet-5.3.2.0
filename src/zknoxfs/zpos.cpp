...
bool CLegacyZKnoxFSStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZKnoxFSStake from non zc-spend", __func__);
    ...
    *this = CLegacyZKnoxFSStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zknoxfs stake origin", __func__);

    // All good
    return true;
}

CLegacyZKnoxFSStake::CLegacyZKnoxFSStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZKnoxFSStake::GetIndexFrom() const
{
...
CAmount CLegacyZKnoxFSStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZKnoxFSStake::GetUniqueness() const
{
...
bool CLegacyZKnoxFSStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zKFX stake block: height %d outside range", __func__, nHeight);
