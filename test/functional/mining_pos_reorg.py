...
class ReorgStakeTest(KnoxFSTestFramework):
    ...
    def check_money_supply(self, expected_knoxfs):
        # verify that nodes have the expected KFX supply
        knoxfs_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(knoxfs_supply, [DecimalAmt(expected_knoxfs)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that KFX supply was properly updated after the reorgs
        self.log.info("Check KFX supply...")
